...
export default {
  ...
  setup() {
    const BOOK_IN_OLD_VIEW_APP_STORE_MODULE_NAME = "book-in-old-view";
    const showBtnAdmin = ref(true);

    // Register module
    if (!store.hasModule(BOOK_IN_OLD_VIEW_APP_STORE_MODULE_NAME))
      store.registerModule(
        BOOK_IN_OLD_VIEW_APP_STORE_MODULE_NAME,
    ...
    onUnmounted(() => {
      if (store.hasModule(BOOK_IN_OLD_VIEW_APP_STORE_MODULE_NAME))
        store.unregisterModule(BOOK_IN_OLD_VIEW_APP_STORE_MODULE_NAME);
    });
    ...
    store
      .dispatch("book-in-old-view/fetchBookIn", {
    ...
    const onDelete = (id) => {
      store
        .dispatch("book-in-old-view/deleteBookIn", { id: id })
        .then((response) => {
