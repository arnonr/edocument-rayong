...
export default {
  ...
  setup() {
    const BOOK_IN_ADD_APP_STORE_MODULE_NAME = "book-in-add";

    // Register module
    if (!store.hasModule(BOOK_IN_ADD_APP_STORE_MODULE_NAME))
      store.registerModule(
        BOOK_IN_ADD_APP_STORE_MODULE_NAME,
        bookInStoreModule
      );
    ...
    onUnmounted(() => {
      if (store.hasModule(BOOK_IN_ADD_APP_STORE_MODULE_NAME))
        store.unregisterModule(BOOK_IN_ADD_APP_STORE_MODULE_NAME);
    });
    ...
    const fetchBookCode = (book_in_category_id) => {
      store
        .dispatch("book-in-add/fetchBookCode", {
    ...
    store
      .dispatch("book-in-add/fetchBookInCategories")
      .then((response) => {
    ...
    const fetchBookTypes = (book_in_category_id) => {
      store
        .dispatch("book-in-add/fetchBookInTypes", {
          book_in_category_id: book_in_category_id,
        })
        .then((response) => {
          const { data } = response.data;
    ...
    store
      .dispatch("book-in-add/fetchDepartments")
      .then((response) => {
        const { data } = response.data;
    ...
    store
      .dispatch("book-in-add/fetchEmailPersons")
      .then((response) => {
        const { data } = response.data;
    ...
    store
      .dispatch("book-in-add/fetchEmailGroups")
      .then((response) => {
        const { data } = response.data;
    ...
    const onSubmit = (ctx, callback) => {
      ...
      store
        .dispatch("book-in-add/addBookIn", dataSend)
        .then((response) => {
