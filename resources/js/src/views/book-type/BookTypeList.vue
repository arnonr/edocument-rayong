      ...
      <b-row>
        <b-col cols="12" md="12">
          <b-button
            variant="success"
            class="mb-sm-0 mr-0"
            @click="$router.push({ name: 'book-type-add' })"
...
import useBookTypeList from "./useBookTypeList";
...
export default {
  ...
  setup() {
    const BOOKTYPE_LIST_APP_STORE_MODULE_NAME = "book-type-list";

    // Register module
    if (!store.hasModule(BOOKTYPE_LIST_APP_STORE_MODULE_NAME))
      store.registerModule(
        BOOKTYPE_LIST_APP_STORE_MODULE_NAME,
        bookTypeStoreModule
      );
    ...
    onUnmounted(() => {
      if (store.hasModule(BOOKTYPE_LIST_APP_STORE_MODULE_NAME))
        store.unregisterModule(BOOKTYPE_LIST_APP_STORE_MODULE_NAME);
    });
