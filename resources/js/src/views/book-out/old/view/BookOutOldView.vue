...
export default {
  ...
  setup() {
    const BOOK_OUT_OLD_VIEW_APP_STORE_MODULE_NAME = "book-out-old-view";
    const showBtnAdmin = ref(true);

    // Register module
    if (!store.hasModule(BOOK_OUT_OLD_VIEW_APP_STORE_MODULE_NAME))
      store.registerModule(
        BOOK_OUT_OLD_VIEW_APP_STORE_MODULE_NAME,
    ...
    onUnmounted(() => {
      if (store.hasModule(BOOK_OUT_OLD_VIEW_APP_STORE_MODULE_NAME))
        store.unregisterModule(BOOK_OUT_OLD_VIEW_APP_STORE_MODULE_NAME);
    });
    ...
    store
      .dispatch("book-out-old-view/fetchBookOut", {
    ...
    const onDelete = (id) => {
      store
        .dispatch("book-out-old-view/deleteBookOut", { id: id })
        .then((response) => {
