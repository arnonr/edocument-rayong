...
export default function useBookOutOld() {
    // Use toast
    ...
    const fetchBookOuts = (ctx, callback) => {
        store
            .dispatch("book-out-old/fetchBookOutsWithYear", {
            ...
            .then((response) => {
                ...
                bookOutLists.value = bookOut.map((item) => {
                    ...
                    book.bookNo =
                        bookNoArr[0].substr(bookNoArr[0].length - 3, 3) +
                        "/" +
                        bookNoArr[1];
    ...
    const fetchYears = (ctx, callback) => {
        store
            .dispatch("book-out-old/fetchYears")
            .then((response) => {
    ...
    const fetchBookTypes = (ctx, callback) => {
        store
            .dispatch("book-out-old/fetchBookTypes")
            .then((response) => {
    ...
    const fetchStatus = (ctx, callback) => {
        store
            .dispatch("book-out-old/fetchBookStatus")
            .then((response) => {
    ...
    const onDelete = (id) => {
        store
            .dispatch("book-out-old/deleteBookOut", { id: id })
            .then((response) => {
    ...
    const onFavoriteChange = (id, favoriteOld) => {
        if (favoriteOld == true) {
            store
                .dispatch("book-out-old/deleteBookOutFavorite", {
                    bookID: id,
            ...
            store
                .dispatch("book-out-old/addBookOutFavorite", {
                    bookID: id,
                    userID: getUserData().userID,
                    bookType: 1,
                })
    ...
    const onChangeStatus = (id, statusID) => {
        store
            .dispatch("book-out-old/changeBookOutStatus", {
                id: id,
                statusID: statusID,
            })
            .then((response) => {
            ...
            items.value = bookOutLists.value.filter((x) => {
                ...
                if (
                    ...
                    if (x.bookTypeName == null) {
                        ...
                        bookTypeNameCheck = x.bookTypeName.includes(
                            advancedSearch.bookTypeName.title
                        );
                    }
                } else {
