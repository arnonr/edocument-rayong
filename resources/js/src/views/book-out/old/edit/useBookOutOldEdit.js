...
export default function useBookOutOldEdit() {
    // Use toast
    ...
    const fetchBookTypes = (ctx, callback) => {
        store
            .dispatch("book-out-old-edit/fetchBookTypesYear",{ yearID: router.currentRoute.params.year})
            .then((response) => {
    ...
    const fetchBookStatus = (ctx, callback) => {
        store
            .dispatch("book-out-old-edit/fetchBookStatus")
            .then((response) => {
