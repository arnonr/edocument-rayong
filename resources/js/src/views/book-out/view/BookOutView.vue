...
export default {
  ...
  setup() {
    const BOOK_OUT_VIEW_APP_STORE_MODULE_NAME = "book-out-view";
    const showBtnAdmin = ref(true);

    // Register module
    if (!store.hasModule(BOOK_OUT_VIEW_APP_STORE_MODULE_NAME))
      store.registerModule(
        BOOK_OUT_VIEW_APP_STORE_MODULE_NAME,
        bookOutStoreModule
      );
    ...
    onUnmounted(() => {
      if (store.hasModule(BOOK_OUT_VIEW_APP_STORE_MODULE_NAME))
        store.unregisterModule(BOOK_OUT_VIEW_APP_STORE_MODULE_NAME);
    });
    ...
    store
      .dispatch("book-out-view/fetchBookOut", {
    ...
    const onDelete = (id) => {
      store
        .dispatch("book-out-view/deleteBookOut", { id: id })
        .then((response) => {
          if (response.data.message == "success") {
            router.push({ name: "book-out-list" });
          } else {
            console.log("error");
          }
        })
        ...
        <b-col>
          ...
          <span class="text-data">
            ...
            <b-button
              ...
              class="btn-icon btn-sm"
              style="margin-bottom: 2px"
              :href="item.book_out_success_file"
              target="_blank"
            >
              <feather-icon icon="FileIcon" style="margin-bottom: -2px" />
              ฉบับสมบูรณ์
