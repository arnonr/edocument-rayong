      ...
      <b-row>
        <b-col cols="12" md="12">
          <b-button
            variant="success"
            class="mb-sm-0 mr-0"
            @click="$router.push({ name: 'department-add' })"
          ...
          >
            ...
            />
            <span class="d-none d-sm-inline">สร้างหน่วยงาน</span>
          </b-button>
          <hr />
        </b-col>
      </b-row>
...
export default {
  ...
  setup() {
    const DEPARTMENT_LIST_APP_STORE_MODULE_NAME = "department-list";

    // Register module
    if (!store.hasModule(DEPARTMENT_LIST_APP_STORE_MODULE_NAME))
      store.registerModule(
        DEPARTMENT_LIST_APP_STORE_MODULE_NAME,
    ...
    onUnmounted(() => {
      if (store.hasModule(DEPARTMENT_LIST_APP_STORE_MODULE_NAME))
        store.unregisterModule(DEPARTMENT_LIST_APP_STORE_MODULE_NAME);
    });
